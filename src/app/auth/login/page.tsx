...
export default function Login() {
    ...
    useEffect(() => {
        ...
        const savedHospitalCode = localStorage.getItem("remembered_hospital_code");
        const savedRememberPref = localStorage.getItem("remember_me_preference");

        if (savedIdentifier) {
            setValue("identifier", savedIdentifier);
        }
        if (savedHospitalCode) {
            setValue("hospital_code", savedHospitalCode);
        }
        if (savedRememberPref === "true" || (savedIdentifier && savedHospitalCode)) {
            setRememberMe(true);
        }
    }, [setValue]);

    const onSubmit = (data: LoginHospitalRequest) => {
        if (rememberMe) {
            localStorage.setItem("remembered_identifier", data.identifier);
            localStorage.setItem("remembered_hospital_code", data.hospital_code);
            localStorage.setItem("remember_me_preference", "true");
        } else {
            localStorage.removeItem("remembered_identifier");
            localStorage.removeItem("remembered_hospital_code");
            localStorage.removeItem("remember_me_preference");
        }

        mutate({ payload: data, rememberMe });
    };
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col w-full max-w-2xl gap-6 px-6">
                ...
                <div className="flex flex-col gap-2">
                    ...
                    <Input
                        type="text"
                        id="hospital_code"
                        placeholder="Masukkan Kode Rumah Sakit (cth: HSP-MO-001)"
                ...
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        ...
                        <Label
                            htmlFor="remember"
                            className="text-base font-normal text-[#212121] cursor-pointer"
                        >
                            Ingat saya
                        </Label>
                    </div>
                ...
                <Button
                    type="submit"
                    className="bg-[#2F907F] hover:bg-[#236C5F] py-6 text-base text-white transition-colors duration-200 cursor-pointer"
                    disabled={isPending}
                >
